...
//----	Project Lana: Fabric, Cloth, and Foram Specified, ARN(S) Mobile Version. 1.00 ----

// Introducing: 
// 1) Albedo (A)
// 2) A linear Specular is introduced
// 3) Roughness (R)
// 4) Normal (N)


Shader "Shader Smart/Fabric | Cloth | Foam/Mobile/ARN(S)" {
	...
	Properties {
		...
		_Spec ("Specular: Uniform Strength", Range(-1,1)) = 0.2

		//----- Roughness -----

		[NoScaleOffset] _RghTex ("Roughness: Map", 2D) = "black" {}
		_RghGray ("Roughness: Gray Level Contrast", Range(0,1)) = 1.0
		_Rgh1 ("Roughness: White Channel Strength", Range(0,1)) = 1.0
		_Rgh0 ("Roughness: Black Channel Strength", Range(0,1)) = 0.0
		_RghAmp ("Roughness: Strength", Range(0,1)) = 1.0
